...
namespace ytail::net {
    ...
    struct BandwidthMeter {
        ...
        // Separate from the window because the graph wants the shape of the traffic, not its average.
        int bytesThisTick = 0;
    ...
    struct EntitySnapshot {
        uint32_t netId = 0;
        // Scene loading preserves ids, so a client matches this to its own copy.
        // TODO only needed the first time a client sees a netId; move it into spawn records along
        // with the component data a runtime-spawned entity needs.
        uint32_t hostEntityId = 0;
        uint32_t ownerPeerId = 0;
        QuantizedState state;
    };

    // One peer's recent input, keyed by shared tick. Reads carry the last frame forward across a
    // gap rather than dropping to neutral. See docs/netcode.md, "Late input".
    struct InputHistory {
        static constexpr size_t Length = 64;
        std::array<NetInputFrame, Length> frames{};
        std::array<Uint64, Length> ticks{};
        Uint64 newestTick = 0;
        // Ticks driven with nothing at all. Repeating the newest frame is not counted, since that is
        // the normal case for anyone but ourselves.
        int starved = 0;
        // Ticks driven with a repeat because the exact frame had not arrived. This is what a missing
        // jump looks like, and why every input bit must mean "held" rather than "pressed just now".
        int repeated = 0;

        // What read() handed back, which is not always what we hold. Keeping it is what lets a late
        // arrival be recognised as a guess we got wrong.
        std::array<NetInputFrame, Length> used{};
        std::array<Uint64, Length> usedTicks{};
        // Earliest tick whose real frame differed from the guess. 0 when there is nothing to redo.
    ...
    // checked against what we already had
    ...
    // How long a packet is held before it is applied, learned from the connection. The measurement
    // is transit, so releasing at a fixed transit spaces packets evenly however they arrived.
    ...
    // Comparing those two is the only honest measure of prediction.
    ...
    // Throttles a log line for a condition that persists rather than an event that happened once.
    // The count of swallowed repeats goes out with the next line, so a rare condition and a constant
    ...
    struct WorldSnapshot {
        // Always behind us, because we run ahead of the host.
        Uint64 tick = 0;
        // Every moving entity is present, making this a complete restore point. False when the
        // budget forced some out, which is safe to apply but not to replay from: the bodies it left
        // out still stand at the tick we have reached. See docs/netcode.md, "Priority and the budget".
        bool complete = true;
        std::vector<EntitySnapshot> entities;
    };

    // What each client is owed. Priority accumulates until an entity is sent, so nothing starves.
    struct ClientSyncState {
        ...
        // How other clients reach this one directly, learned from its PeerHello.
        uint64_t peerAddress = 0;

        // All indexed by netId, grown on demand.
        std::vector<float> priority;
        std::vector<QuantizedState> lastSentState;
        // Owner is not part of the state, so a handover on a still body would otherwise go unsent.
        std::vector<uint32_t> lastSentOwner;
        // Sends still owed for an entity whose state stopped changing, so a settled body does not go
        // quiet on the very packet that announced it settled.
    ...
    // host as the clock. Clients run ahead of it, send input rather than positions, and snap
    // received state onto the running simulation.
    //
    // Owned by Engine, inert until bind() supplies a peer.
    // See docs/netcode.md for the model, the wire format, and how the three delays interact.
    ...
    public:
        ...
        void drawNetGraph();
        // Meant for the top of the debug window, so the graph is reachable without scrolling.
        ...
        // peers' traces on (tick, netId) gives the prediction error directly. Consumed by
        // tools/nettest.py. No-op until a path is supplied.
        ...
        // What the local player pressed this tick, filed under our own peer id.
        ...
        // False means a tick we have not lived through yet, so the controls should be read. True
        // means a replay is passing back over one, and reading again would rewrite what was pressed.
        [[nodiscard]] bool hasLocalInputFor(Uint64 tick) const;

        // The host is a peer like any other, so its input is looked up and forwarded the same way.
        static constexpr uint32_t NoOwner = 0;
        static constexpr uint32_t HostPeerId = 1;

        // reasoning in docs/netcode.md.
        ...
        // it gets to reach the other peers.
        ...
        // which costs a hop and so a worse prediction of the other players.
        static bool peerMesh;
        static Uint64 maxInputDelayTicks;
        // How far a body may sit from where the host put it before the world is rewound and
        // replayed. Position and angle are independent because they diverge independently.
        ...
        // Input goes out every tick. It is a handful of bits, and every peer's simulation of every
        // other player depends on it.
        static constexpr Uint64 InputSendInterval = 1;
        // A "that peer is gone" threshold, not a latency one. At 300ms a remote player's newest
        // input is routinely 20 ticks behind the tick being simulated.
        static constexpr Uint64 MaxInputStaleTicks = 60;
        static constexpr Uint64 CorrectionMarkerTicks = 45;
        // Below this a correction is normal quantization noise and is not worth drawing.
        ...
        // Long enough for the previous adjustment to have made the round trip and shown up in the
        // reported lead.
        ...
        // Ticks we are willing to replay in one go. Past this, snapping and carrying on costs less
        // than the catch-up. Has to exceed the worst depth a supported connection produces, which is
        // about 30 ticks at 300ms.
        ...
        static_assert(MaxRollbackTicks < InputHistory::Length,
                      "rollback window exceeds the input history ring");
        // Both bound how stale the input driving a body can get.
        static_assert(MaxInputLeadTicks + MaxJitterTicks < InputHistory::Length,
                      "clock lead plus jitter hold exceeds the input history ring");
    ...
    private:
        ...
        // Host to clients whenever the membership changes, so each one can dial the others.
        void sendPeerRoster();
        ...
        // True when every body in the packet was already within tolerance of where the host puts it,
        // so there is nothing a replay could improve.
        ...
        // buffered input. force skips the agreement check, for a replay driven by input rather than
        // by state.
        ...
        // differ. Replays from the last authoritative state we have.
        ...
        // Reused so a snapshot costs no allocation, and word sized because BitWriter/BitReader spill
        // whole words. receiveWords also pads a message up to a word boundary.
        ...
        // The jitter buffer, sorted so snapshots are released in the order the host produced them.
        ...
        // steadily is the whole system working.
        int predictionsKept = 0;
        int predictionsChecked = 0;
        // Depth times rate is the extra physics being run, against the 60 steps a second a tick
        // normally costs.
        int lastResimDepth = 0;
        int deepestResim = 0;
        int resimStepsThisWindow = 0;
        float resimStepsPerSecond = 0.0f;
        // Corrections applied without a replay, which leaves the world a rollback's worth of time in
        // the past.
        ...
        int starvedTotal = 0;
        // Counted from gaps in the tick numbers rather than from the backend, so it measures what
        // the simulation actually went without.
        int lostSnapshots = 0;
